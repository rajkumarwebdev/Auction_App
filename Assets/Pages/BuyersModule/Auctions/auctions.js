...
const searchByBikeName = document.getElementById("searchByBikeName");
const stateEl = document.getElementById("state");
const cityEl = document.getElementById("district");
const brandNamesEl = document.getElementById("brandNames");
const filterResultSpanEl = document.getElementById("filter-result");
const filterResultHolder = document.getElementById("filterResults");
const sortingInput = document.getElementById("sortingInput");
const container = document.querySelector(".container");
const userID = localStorage.getItem("_id");

const bestSellerCount = 10;
...
function loadBrands() {
  ...
  opt.value = null;
  opt.innerText = "Choose Brand";
  opt.selected = true;
  opt.disabled = true;
  brandNamesEl.appendChild(opt);
...
function loadCurrentAuctions(searchString, code) {
  loading();
  auctionListings.innerHTML = "";
  let acc = getAuctionSystem();
  let auctionBikes = acc["auctions"];
  let user = acc["users"].find((user) => user.userID == userID);

  auctionBikes = auctionBikes.filter((auction) => auction.status == "Active");
  const bikeOriginal = getAuctionSystem()["items"];
  //   console.log(auctionBikes);

  // filter later
  console.log(searchString);

  if (searchString) {
    auctionBikes = auctionBikes.filter((auctionBike) => {
      let bikeObj = bikeOriginal.find((b) => b.id == auctionBike.itemId);
      console.log(bikeObj);
      return bikeObj.name.toLowerCase().includes(searchString.toLowerCase());
    });
  }
  if (code && Object.keys(code)[0] == 1) {
    console.log("ama");
    let bikes = getAuctionSystem()["items"];
    auctionsIds = bikes
      .filter((bike) => bike.brand == code[1])
      .map((bike) => bike.auctionId);
    auctionBikes = auctionBikes.filter((auction) =>
      auctionsIds.includes(auction.id)
    );
  }
  if (code && Object.keys(code)[0] == 2) {
    let bikes = getAuctionSystem()["items"];
    auctionsIds = bikes
      .filter((bike) => bike.state == stateEl.value && bike.city == code[2])
      .map((bike) => bike.auctionId);
    auctionBikes = auctionBikes.filter((auction) =>
      auctionsIds.includes(auction.id)
    );
  }
  if (code && Object.keys(code)[0] == 3) {
    if (code[3] == "lowest rate") {
      auctionBikes = auctionBikes.sort(
  ...
  // showing the filter result------------------------------
  if (code) {
    let result = `${Object.values(code)[0]} bikes...`;
    filterResultSpanEl.innerText = result;
    filterResultHolder.classList.add("show");
  } else {
    filterResultHolder.classList.remove("show");
  }

  if (auctionBikes.length > 0) {
    auctionBikes.forEach((bike) => {
      //i am getting best seller
      let isBestSeller;
      let thisUserAuctions = acc["auctions"].filter(
        (auction) => auction.sellerId == bike.sellerId
      );
      isBestSeller = true;
      let closedAuctionCount = thisUserAuctions.filter(
        (auction) => auction.status == "Closed"
      ).length;
      let rejectedAuctionCount = thisUserAuctions.filter(
        (auction) => auction.status == "Rejected"
      ).length;
      if (rejectedAuctionCount == 0 && closedAuctionCount >= 1) {
        isBestSeller = true;
      } else {
        let ratio = closedAuctionCount / rejectedAuctionCount;
        if (ratio > 1) {
          isBestSeller = true;
        } else {
          isBestSeller = false;
      ...
      auctionCard.setAttribute("data-auctionId", bike.id);
      let bikeObj = bikeOriginal.find((b) => b.id == bike.itemId);
      let bidded;
      if (user["auctionsParticipating"].includes(bike.id)) {
        bidded = true;
      } else {
        bidded = false;
      }
      ...
      let item = `
          <img src=${bikeObj.image} alt="Bike Image" />
            ${bidded ? '<p class="auctionTag">You have bidded</p>' : ""}
            ${isBestSeller ? '<p class="auctionTag best">Best seller</p>' : ""}
          ...
          <div class="card-details">
            ...
            <div class="card-info">
              ...
              <div class="price">Starting : &#8377;${bike.startingPrice}</div>
              <div class="current-bid">Current : &#8377;${
                bike.currentPrice
              }</div>
            </div>
            <div class="remaining-time">Time Left: <span class="countdown">0</span></div>
            <a href="/Auction_App/Assets/Pages/BuyersModule/MyBids/mybids.html?id=${
              bike.id
            }" class="place-bid-btn">Place Bid</a>
          </div>`;
      ``;
      auctionCard.innerHTML = item;
      auctionListings.appendChild(auctionCard);
    });
    setTimeout(() => {
      stopLoading();
    }, 500);
...
searchByBikeName.addEventListener("keyup", (e) => {
  searchBike(e);
});

function searchBike(e) {
  let searchString = e.target.value;
  loadCurrentAuctions(searchString);
}

stateEl.addEventListener("change", (e) => populateCities(e));

function populateCities(e) {
  let state = e.target.value;
  let cities = states.filter((stat) => stat.state == state);
  cityEl.innerText = "";
  const opt = document.createElement("option");
  opt.value = null;
  opt.innerText = "Choose city";
  opt.selected = true;
  opt.disabled = true;
...
function addStates() {
  let stateNames = [...new Set(states.map((state) => state.state))];
  const opt2 = document.createElement("option");
  opt2.value = null;
  opt2.innerText = "Choose state";
  opt2.selected = true;
  opt2.disabled = true;
  stateEl.appendChild(opt2);
...
brandNamesEl.addEventListener("change", (e) => {
  loadCurrentAuctions(null, { 1: e.target.value });
});

cityEl.addEventListener("change", (e) => {
  loadCurrentAuctions(null, { 2: e.target.value });
});

sortingInput.addEventListener("change", (e) => {
  loadCurrentAuctions(null, { 3: e.target.value });
});
