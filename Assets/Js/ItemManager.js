const imageInput = document.getElementById("image_url_bike");
const loadingSpinnerr = document.getElementById("loading-spinner");

imageInput.addEventListener("change", function (event) {
  const file = event.target.files[0];
  //loading start
  loadingSpinnerr.classList.add("show");
  if (file && file.type.startsWith("image/")) {
    ...
    reader.onload = function (e) {
      ...
      async function uploadImage(file) {
        ...
        formData.append("upload_preset", "my_img_raj_007");
        const response = await fetch(
          "https://api.cloudinary.com/v1_1/dye2p5i78/image/upload",
          {
            method: "POST",
            body: formData,
          }
      ...
      uploadImage(base64Image).then((url) => {
        ...
        // loading end
        loadingSpinnerr.classList.remove("show");
      });
    };
...
const addBikeBtn = document.getElementById("add-item");
const modelIpt = document.getElementById("modelIpt");
const brandIpt = document.getElementById("brandIpt");
const yearIpt = document.getElementById("yearIpt");
const engineCapacityIpt = document.getElementById("EngineCapacityIpt");
const ownerEle = document.getElementById("owner");
const imageEle = document.getElementById("imagePreview");
const stateEl = document.getElementById("state");
const cityEl = document.getElementById("district");

const addBikeBtnAction = document.getElementById("add-bike-btn");
addBikeBtnAction.addEventListener("click", storeBikeData);
// validate bike form
function validateBikeData(bikeName, owner, image, state, city) {
  console.log(state, city);

  let errs = {};
  if (!state || state == null || state == "") {
    console.log("wgy");
    errs.err_state = "Please choose state name!";
  }
  if (!city || city == null || city == "") {
    errs.err_city = "Please choose city name!";
  }
  if (!bikeName) {
    errs.err_name = "Please fill bike name!";
  }
  if (!owner) {
    errs.err_owner = "Please give number of owners of this bike!!";
  }
  if (!image) {
    errs.err_image = "Please!Choose bike image.";
  }
...
function storeBikeData() {
  ...
  let model = modelIpt.value.trim();
  let brand = brandIpt.value.trim();
  let year = yearIpt.value.trim();
  let owner = ownerEle.value.trim();
  let image = imageEle?.firstChild?.src;
  let capacity = engineCapacityIpt.value.trim();
  let state = stateEl.value;
  // console.log(stateEl);

  let city = cityEl.value;
  if (validateBikeData(bikeName, owner, image, state, city)) {
    ...
    // let auction = createAuction(crypto.randomUUID(), item.id, userID, endDate);
...
addBikeBtn.addEventListener("click", loadBikeDetails);
...
function clearInputs() {
  ...
  brandIpt.value = "";
  yearIpt.value = "";
  engineCapacityIpt.value = "";
  ownerEle.value = "";
  imageEle.src = "";
}

function loadBikeDetails() {
  console.log(bikeDetails.length);
  // Adding bike names
  const opt = document.createElement("option");
  opt.value = null;
  opt.innerText = "Choose Bike";
  opt.selected = true;
  opt.disabled = true;
...
bikeNamesInput.addEventListener("change", (e) => populateValues(e));

function populateValues(e) {
  let currentValue = e.target.value;
  let bike = bikeDetails.find((bike) => bike.bikeName == currentValue);
  modelIpt.value = bike.model;
  brandIpt.value = bike.brand;
  yearIpt.value = bike.year;
  engineCapacityIpt.value = bike.engineCapacity;
}
...
function populateCities(e) {
  ...
  let cities = [
    ...new Set(
      states.filter((stat) => stat.state == state).map((city) => city.city)
    ),
  ];
  cityEl.innerText = "";
  const opt = document.createElement("option");
